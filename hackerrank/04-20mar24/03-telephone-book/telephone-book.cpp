...
/**
  Method useful to order a vector of strings in alpabeth order.
  Will return true if fist string goes before second string.
*/
bool order_swap_strings(const std::string & str1, const std::string & str2){
  int i = 0;
  while((int)str1.at(i) == (int)str2.at(i) && i < std::min(str1.size(), str2.size())) i++;
...
class Rubrica {
  private:
    ...
    void save_contacts(const std::string & input){
      ...
      for(int i = 0; i < parts.size(); i += 2) {
        m_contacts.push_back(Contatto{parts.at(i), parts.at(i + 1)});
      }
    }
    ...
    void order_contacts(){
      ...
      do {
        ...
        for(int i = 0; i < m_contacts.size() - 1; i++){
          if (order_swap_strings(m_contacts.at(i).m_fullname, m_contacts.at(i + 1).m_fullname)) {
            changes_made = true;
            const Contatto i_contact = m_contacts.at(i);
            m_contacts.at(i) = m_contacts.at(i + 1);
            m_contacts.at(i + 1) = i_contact;
          }
        }
      } while (changes_made);
    }
  ...
  public:
    std::vector<Contatto> m_contacts;
