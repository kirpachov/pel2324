...
void print_hashmap(std::unordered_map<std::string, long long int> const &hashmap){
        for (const auto &p : hashmap) {
                std::cout << p.first << '\t' << p.second << "\n";
        }
}

void print_hashmap(std::unordered_map<int, std::string> const &hashmap){
        for (const auto &p : hashmap) {
                std::cout << p.first << '\t' << p.second << "\n";
        }
}
...
/**
 * Extract instruction type from full instruction
 */
std::string ctype(const std::string instruction){
	return split(instruction, " ").at(0);
}
...
//	std::cout << "Processing '" << instruction << "' (" << parts.at(0) << ") " << std::endl;
...
int main(void){
	...
	do {
		...
		/*std::cout << line - 1 << ") " << instruction << std::endl;
		print_hashmap(data);
		std::cout << "\n\n";*/
